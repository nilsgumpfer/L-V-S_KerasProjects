...
nb_train_samples = 8528
nb_test_samples = 8528
...
model = Sequential()
model.add(Conv2D(nb_filters, (16, 16), input_shape=input_shape, padding='valid'))
model.add(Activation('relu'))
model.add(MaxPool2D(pool_size=(2, 2)))

model.add(Conv2D(nb_filters, (8, 8)))
model.add(Activation('relu'))
model.add(MaxPool2D(pool_size=(2, 2)))

model.add(Conv2D(nb_filters, (4, 4)))
model.add(Activation('relu'))
model.add(MaxPool2D(pool_size=(2, 2)))
model.add(Dropout(0.25))
...
with open('optimized_model.json', 'w') as model_file:
...
# model.load_weights('optimized_weights_run1.h5')
...
model.save_weights('optimized_weights_run1.h5')
